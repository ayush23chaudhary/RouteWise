...
class TripService:
    ...
    def plan_trip(
        ...
        """
        # 1. Validate Input Coordinates & Domain Constraints
        ...
        if initial_cycle_used_seconds < 0 or initial_cycle_used_seconds > 252000:
            raise ValueError("Initial cycle used seconds must be between 0 and 252,000 seconds (70 hours).")

        # 2. Build Initial Driver HOS State & Trip Aggregate
        trip_id = uuid.uuid4()
        ...
        # 3. Add Waypoints (Start, Pickup = 1hr dwell, Dropoff = 1hr dwell)
        trip.add_waypoint(
            DomainWaypoint(
                id=uuid.uuid4(),
                sequence=1,
        ...
        trip.add_waypoint(
            DomainWaypoint(
                ...
                coordinates=pickup,
                duration_seconds=3600,  # 1 hour loading requirement
            )
        )
        trip.add_waypoint(
            DomainWaypoint(
                ...
                coordinates=dropoff,
                duration_seconds=3600,  # 1 hour unloading requirement
            )
        )

        # 4. Fetch Route distance & duration from Geospatial Service
        ...
        # 6. Save and return confirmed domain aggregate
