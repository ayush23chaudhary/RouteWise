...
def custom_exception_handler(exc: Exception, context: dict[str, Any]) -> Response | None:
    ...
    if response is not None:
        ...
        # Fallback for unhandled server exceptions (500 Internal Server Error)
        request = context.get("request")
        correlation_id = getattr(request, "correlation_id", str(uuid.uuid4())) if request else str(uuid.uuid4())
